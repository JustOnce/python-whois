...
class Domain:
    ...
    def __init__(self, data):
        self.name = data.get('domain_name', [''])[0].strip().lower()
        self.registrar	= data.get('registrar', [''])[0].strip()
        self.registrant	= data.get('registrant', [''])[0].strip()

        try:
            self.creation_date = str_to_date(data.get('creation_date')[0])
        except ValueError as ex:
            self.creation_date = None

        try:
            self.expiration_date = str_to_date(data.get('expiration_date')[0])
        except ValueError as ex:
            self.expiration_date = None

        try:
            self.last_updated = str_to_date(data.get('updated_date')[0])
        except ValueError as ex:
        ...
        # name_servers
        tmp = []
        for x in data.get('name_servers', []):
...
def str_to_date(s):
    ...
    s = s.replace('(jst)', '+0900').replace('(JST)', '+0900')
    ...
    except ValueError: raise ValueError("Unknown date format: '%s'" % s)
